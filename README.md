...
| Add prewedding photos | drop portrait shots (`tenis-1.jpg`…) or landscape shots (`tenis-1-ls.jpg`…) into `assets/img/gallery/` — see `assets/img/gallery/README.md`. The photo count is dynamic: anything not uploaded yet is just skipped, no placeholder tile |
...
- [x] Prewedding gallery — 3 concepts (tenis, taman, museum), auto-scrolling portrait + landscape rows with a dynamic photo count (no placeholders for photos not uploaded yet), and a lightbox
